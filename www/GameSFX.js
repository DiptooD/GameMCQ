class GameSFX {
    ...
    static init(scene) {
        ...
        registerSound('sfx_enemy_dash', 
            this.createEnemyDash(ctx));
            
        registerSound('sfx_enemy_shield_hit', 
            this.createEnemyShieldHit(ctx));
            
        registerSound('sfx_enemy_shield_break', 
            this.createEnemyShieldBreak(ctx));
            
        registerSound('sfx_enemy_bomb', 
            this.createEnemyBomb(ctx));
    ...
    static createEnemyBomb(ctx) {
        ...
        for(let i=0; i<length; i++) {
            ...
            let noise = Math.random() * 2 - 1;
            noise = lastOut + 0.05 * (noise - lastOut); // LPF for dark boom
            lastOut = noise;
